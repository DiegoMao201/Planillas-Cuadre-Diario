...
            # Se agrupa para obtener el valor TOTAL del recibo, asumiendo que el valor del recibo
            # está en la columna 'Valor Efectivo', repetido por cada línea de factura.
            # Se usa 'first' en lugar de 'sum' para evitar duplicación.
            individual_grouped = df_individual.groupby('Recibo N°').agg(
                Valor_Total=('Valor Efectivo', 'first'), 
                Fecha=('Fecha', 'first'),
...
            # Aquí SÍ se suma, porque los recibos se están CONSOLIDANDO por Agrupación y Destino
            # (la suma de los "valores de recibo" agrupados dará el total de la consignación).
...
            # Para el total del día, SÍ se debe sumar el valor de todas las filas del día,
            # pero antes se debe asegurar que se use solo el valor único del recibo
            # para evitar duplicaciones por líneas de factura.
            # Aquí, agrupamos por Recibo N° y obtenemos el primer valor, luego sumamos esos valores únicos.
            daily_recibo_values = group_df.groupby('Recibo N°')['Valor Efectivo'].first().sum()

            total_dia = daily_recibo_values # Utilizamos el valor calculado correctamente
...
    Ordena primero por Fecha para manejar reportes de múltiples días.
    ...
    NOTA: Este reporte usa el df completo (detalle por factura) para mostrar los totales,
    pero los subtotales deben usar el valor único del recibo.
    """
    output = BytesIO()
    
...
            
            # Subtotal usa el primer valor de 'Valor Efectivo', asumiendo que el valor del recibo
            # se repite en cada fila (para evitar el doble conteo).
            subtotal = group['Valor Efectivo'].iloc[0] 
...
                report_data.append(row.tolist())
            
            # Subtotal de la consignación: Suma de los valores de recibo únicos dentro del grupo.
            # Agrupar por recibo y obtener el primer valor para cada uno, y luego sumar esos.
            subtotal = group.groupby('Recibo N°')['Valor Efectivo'].first().sum() 
...
        # Calcular el total general usando el valor único de cada recibo.
        grand_total = df.groupby('Recibo N°')['Valor Efectivo'].first().sum() 
...
                                    
                                    # Preparar el DataFrame para la edición. Mantenemos solo un registro por Recibo N° con el valor TOTAL (no la suma de detalles).
...
                                    
                                    # CORRECCIÓN DE LA SUMA DOBLE: Se usa 'first' en lugar de 'sum'
                                    df_summary_edit = filtered_df.groupby('Recibo N°').agg(
                                        Fecha=('Fecha', 'first'),
                                        Cliente=('Cliente', 'first'),
                                        Valor_Efectivo_Total=('Valor Efectivo', 'first'), # <--- CORREGIDO: Usar 'first'
...
        
        # El total del grupo se calcula correctamente tomando el valor único de cada recibo.
        total_efectivo_grupo = st.session_state.df_full_detail.groupby('Recibo N°')['Valor Efectivo'].first().sum()
        st.metric(label="💰 Total Efectivo del Grupo", value=f"${total_efectivo_grupo:,.2f}")
        
