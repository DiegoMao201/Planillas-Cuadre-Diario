                ...
                <td colspan="2">
                    <div style="background-color:#fff; border-radius:8px; border:1px solid #e0e0e0; padding:12px; margin:0 12px 12px 12px;">
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" width="100%">
                            ...
                            <tr>
                                <td style="padding:2px 0; font-size:14px; color:#555; border-top:1px dashed #ccc;">Diferencia</td>
...
# ======================================================================================
# --- NUEVA SECCIÓN: ENVÍO DE CORREO GERENCIAL CON FORMATO HTML PERSONALIZADO ---
# ======================================================================================

def generate_manager_email_body(records, start_date, end_date, selected_store):
    """
    Genera el cuerpo de un correo electrónico con el diseño del HTML proporcionado por el usuario.
    """
    report_time_str = datetime.now().strftime("%d/%m/%Y a las %H:%M:%S")
    ...
    # Construcción de los detalles por tienda/fecha
    details_html = ""
    total_balance = 0

    # Calcular Saldo Total (esto es un poco diferente al cuadre diario)
    # Aquí sumaremos todos los ingresos y restaremos los egresos del período
    for record in records:
        tarjetas_data = json.loads(record.get('Tarjetas', '[]'))
        consignaciones_data = json.loads(record.get('Consignaciones', '[]'))
        gastos_data = json.loads(record.get('Gastos', '[]'))
        efectivo_data = json.loads(record.get('Efectivo', '[]'))

        ingresos = sum(item.get('Valor', 0) for item in tarjetas_data)
        ingresos += sum(item.get('Valor', 0) for item in consignaciones_data)
        ingresos += float(record.get('Venta_Total_Dia', 0)) - (sum(item.get('Valor', 0) for item in tarjetas_data) + sum(item.get('Valor', 0) for item in consignaciones_data)) # La parte de efectivo de la venta
        
        egresos = sum(item.get('Valor', 0) for item in gastos_data)
        egresos += sum(item.get('Valor', 0) for item in efectivo_data if item.get('Tipo') == "Efectivo Entregado")

        total_balance += ingresos - egresos


    # Construir la sección de movimientos detallados para el correo (igual al HTML original)
    for record in records:
        fecha_str = record.get('Fecha', 'N/A')
        tarjetas_data = json.loads(record.get('Tarjetas', '[]'))
        consignaciones_data = json.loads(record.get('Consignaciones', '[]'))
        ...
        # Combine all transaction types for the table
        all_transactions = []
        all_transactions.extend([
            { 'date': item.get('Fecha', fecha_str), 'concept': 'Venta Tarjeta', 'type': 'ingreso', 'amount': item.get('Valor', 0) } for item in tarjetas_data
        ])
        all_transactions.extend([
            { 'date': item.get('Fecha', fecha_str), 'concept': f'Consignación {item.get("Banco")}', 'type': 'ingreso', 'amount': item.get('Valor', 0) } for item in consignaciones_data
        ])
        all_transactions.extend([
            { 'date': fecha_str, 'concept': item.get('Descripción'), 'type': 'egreso', 'amount': item.get('Valor', 0) } for item in gastos_data
        ])
        all_transactions.extend([
            { 'date': fecha_str, 'concept': f'{item.get("Tipo")} a {item.get("Destino/Tercero (Opcional)", "N/A")}', 'type': 'egreso', 'amount': item.get('Valor', 0) } for item in efectivo_data if item.get('Tipo') == "Efectivo Entregado"
        ])
        all_transactions.extend([
            { 'date': fecha_str, 'concept': item.get("Tipo"), 'type': 'ingreso', 'amount': item.get('Valor', 0) } for item in efectivo_data if item.get('Tipo') == "Reintegro Caja Menor"
        ])

        for t in sorted(all_transactions, key=lambda x: x['date'], reverse=True):
            details_html += f"""
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    <td style="padding: 12px; color: #4b5563;">{t['date']}</td>
                    <td style="padding: 12px; color: #4b5563;">{t['concept']}</td>
                    <td style="padding: 12px;">
                        <span style="font-size: 11px; font-weight: 500; padding: 4px 8px; border-radius: 9999px; background-color: {'#d1fae5' if t['type'] == 'ingreso' else '#fee2e2'}; color: {'#065f46' if t['type'] == 'ingreso' else '#991b1b'};">
                            {t['type'].capitalize()}
                        </span>
                    </td>
                    <td align="right" style="padding: 12px; font-weight: 700; color: {'#10b981' if t['type'] == 'ingreso' else '#ef4444'};">
                        {'+' if t['type'] == 'ingreso' else '-'}{format_cop(t['amount'])}
                    </td>
                </tr>
            """


    # HTML principal con los datos calculados
    html_body = f"""
    <!DOCTYPE html>
    <html lang="es">
    <head>
        ...
        <style>
            body {{ font-family: 'Roboto', sans-serif; background-color: #f8f9fa; margin: 0; padding: 0; }}
            .container {{ max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.05); overflow: hidden; border: 1px solid #dee2e6; }}
            .header {{ background: linear-gradient(90deg, #023e8a, #0077b6); color: #ffffff; padding: 24px; text-align: center; }}
            .header h1 {{ margin: 0; font-size: 24px; font-weight: 700; }}
            .header p {{ margin: 5px 0 0; font-size: 14px; color: #caf0f8; }}
            .summary-cards {{ padding: 24px; display: flex; justify-content: space-between; gap: 16px; flex-wrap: wrap; }}
            .summary-card {{ flex: 1; min-width: 150px; background-color: #ffffff; border-radius: 8px; border: 1px solid #e0e0e0; padding: 16px; text-align: left; }}
            .summary-card p {{ margin: 0; font-size: 14px; color: #6c757d; }}
            ...
            th {{ background-color: #f1f5f9; text-align: left; padding: 12px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; color: #4b5563; }}
            td {{ padding: 12px; font-size: 14px; color: #6b7280; }}
            .footer {{ background-color: #f1f3f5; padding: 16px; text-align: center; font-size: 12px; color: #6c757d; }}
        </style>
    </head>
    <body>
        <div class="container">
            <div class="header">
                ...
                <p>Dashboard Interactivo - {selected_store}</p>
                <p style="font-size: 12px; margin-top: 10px;">{start_date.strftime('%d/%m/%Y')} a {end_date.strftime('%d/%m/%Y')}</p>
            </div>
            
            <div class="summary-cards">
                <div class="summary-card">
                    <p>Saldo Total</p>
                    <div class="value balance">{format_cop(total_balance)}</div>
                </div>
                <div class="summary-card">
                    <p>Ingresos del Período</p>
                    <div class="value income">{format_cop(sum(float(r.get('Venta_Total_Dia', 0)) for r in records))}</div>
                </div>
                <div class="summary-card">
                    <p>Egresos del Período</p>
                    <div class="value expenses">{format_cop(sum(sum(float(g.get('Valor', 0)) for g in json.loads(r.get('Gastos', '[]'))) for r in records) + sum(sum(float(e.get('Valor', 0)) for e in json.loads(r.get('Efectivo', '[]')) if e.get('Tipo') == 'Efectivo Entregado') for r in records))}</div>
                </div>
            </div>

            <div class="content">
                <h2 style="font-size: 18px; color: #023e8a; margin-top: 0; border-bottom: 2px solid #0077b6; padding-bottom: 8px;">Detalle de Movimientos</h2>
                <div class="table-container">
                    <table style="width: 100%;">
                        <thead>
                            <tr>
                                <th style="border-right: 1px solid #e5e7eb;">Fecha</th>
                                <th style="border-right: 1px solid #e5e7eb;">Concepto</th>
                                <th style="border-right: 1px solid #e5e7eb;">Tipo</th>
            ...
            <div class="footer">
                Dashboard generado automáticamente el {report_time_str}.<br>
                <strong>Sistema de Gestión Financiera</strong>
            </div>
        </div>
    </body>
    </html>
...
def send_manager_email_report(registros_ws, start_date, end_date, selected_store, recipient_email):
    """
    Prepara y envía el correo con el reporte gerencial usando el diseño del HTML.
    """
    st.info("Preparando y enviando reporte gerencial con diseño profesional...")
    try:
        sender_email = st.secrets["email_credentials"]["sender_email"]
        sender_password = st.secrets["email_credentials"]["sender_password"]
    except (KeyError, TypeError):
    ...
    # Genera el cuerpo del correo con el nuevo diseño
    email_body = generate_manager_email_body(filtered_records, start_date, end_date, selected_store)
    subject = f"Reporte Gerencial Diario - {selected_store} - {start_date.strftime('%d/%m')} a {end_date.strftime('%d/%m')}"
    ...
    try:
        ...
        st.success(f"¡Reporte gerencial con diseño profesional enviado exitosamente a {recipient_email}!")
    except smtplib.SMTPAuthenticationError:
        st.error("Error de autenticación con el servidor de correo. Verifique el email y la contraseña de aplicación en los 'secrets'.")
    except Exception as e:
        st.error(f"Ocurrió un error inesperado al enviar el correo: {e}")
...
def render_reports_page(registros_ws, config_ws, tiendas_list):
    ...
    # Botones de reporte original
    st.subheader("Opciones de Reporte Original", anchor=False)
    ...
    with b3:
        with st.form("email_form"):
            recipient_email = st.text_input("Email del Gerente (diseño compacto)", placeholder="ejemplo@dominio.com")
    ...
    # Nuevo bloque de reporte con diseño HTML
    st.subheader("Reporte Gerencial con Diseño HTML", anchor=False)
    
    with st.form("new_email_form"):
        recipient_email_html = st.text_input("Email del Gerente (diseño HTML)", placeholder="ejemplo@dominio.com")
        submitted_html = st.form_submit_button("📧 Enviar Reporte en HTML", use_container_width=True)
        
        if submitted_html:
            if recipient_email_html and "@" in recipient_email_html:
                send_manager_email_report(registros_ws, start_date, end_date, selected_store, recipient_email_html)
            else:
                st.warning("Por favor, ingrese una dirección de correo válida.")
